# from https://github.com/damaru2/ankigenbot/blob/master/src/database.py
...
class AnkiGenDB:
    ...
    def update_deck_name(self, id_chat, deck, lang):
        if self.get_deck_name(id_chat, lang.value) is not None:
            update_query = (
                """UPDATE deck_names SET deck = ? WHERE id_chat = ? and language = ?"""
            )
    ...
    def update_word_lang(self, id_chat, lang, word_lang):
        ...
        if self.get_deck_name(id_chat, lang.value) is not None:
            ...
            self.conn.execute(
                update_query, (word_lang if word_lang else "", id_chat, lang.value)
            )
        else:
    ...
    def update_def_lang(self, id_chat, lang, def_lang):
        ...
        if self.get_deck_name(id_chat, lang.value) is not None:
            ...
            self.conn.execute(
                update_query, (def_lang if def_lang else "", id_chat, lang.value)
            )
        else:
    ...
    def update_tags(self, id_chat, tags, lang):
        if self.get_tags(id_chat, lang.value) is not None:
            update_query = (
                """UPDATE deck_names SET tags = ? WHERE id_chat = ? and language = ?"""
            )
    ...
    def insert_new_user(
        ...
        self.conn.execute(
            insert_new_user,
            (id_chat, username, password, language, state, reverse, ipa, card_type),
        )
        self.conn.commit()
